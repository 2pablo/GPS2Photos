 ...
 * @param int    $idn The attachment ID.
 ...
 */
function gps2photos_get_map_for_modal( $options, $idn, $file_path = null, $gps_data ) {
...
<div id="gps2photos-modal-' . $idn . '" class="gps2photos-modal">
	<div class="gps2photos-modal-content" style="max-width: ' . $options['map_width'] . '; max-height: ' . $options['map_height'] . ';">
		...
		<div id="gps2photos-modal-message-' . $idn . '" class="gps2photos-modal-message" style="display: none; margin: 5px 0 15px;"></div>
	...
	$output           .= '
		<p><button type="button" class="button button-primary gps2photos-save-coords-btn" data-image-id="' . $idn . '" data-original-lat="' . esc_attr( $lat ) . '" data-original-lon="' . esc_attr( $lon ) . '" data-file-path="' . esc_attr( $file_path ) . '">' . esc_html__( 'Save Coordinates', 'gps-2-photos' ) . '</button></p>
		<p><button type="button" class="button gps2photos-restore-coords-btn"
						data-image-id="' . $idn . '" data-file-path="' . esc_attr( $file_path ) . '" ' . $restore_btn_style . '>' . $restore_btn_text . '</button></p>
		</div>
	</div>
</div>
<script>
    ...
    function gps2photos_init_map_' . $idn . '(apiKey, deferred) {
        // Prevent re-initialization.
        if (window.gps2photos_maps[' . $idn . ']) {
            if (deferred) {
                deferred.resolve(window.gps2photos_maps[' . $idn . ']); // Resolve immediately if already initialized
            }
            return;
        }
        ...
        // Get input fields for later use (they might be empty initially for NextGEN)
        ...
        // Initial map center and zoom (will be updated by AJAX if coordinates exist)
        var initialLat = 30; // Default center
        var initialLon = 0;  // Default center
	...
	$output .= '
        ...
        map.events.add("ready", function () {
            ...
            map.controls.add([
                        ...
                        position: "top-left" // Options: "top-left", "top-right", "bottom-left", "bottom-rght", "non-fixed"
                });i
            var marker;
            ...
            if (deferred) {
                deferred.resolve(map); // Resolve the deferred object when the map is truly ready
            }
        });
    }
